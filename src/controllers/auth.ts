...
export const createNewUser: RequestHandler =  async (req, res) => {
  ...
  if (existingUser) 
    return sendErrorRes(res, "User already exists, email is alreay in use!", 401)
    ...
    res.json({ messsage: "Plase check your inbox"})
...
export const verifyEmail: RequestHandler = async (req, res) => {
  ...
  if (!authToken) 
    return sendErrorRes(res, "Unauthorized resquest", 403)

  const isMatched = await authToken.compareToken(token)
  if (!isMatched)
    return sendErrorRes(res, "Unauthorized resquest, invalid token", 403)

  await UserModel.findByIdAndUpdate(id, { verifed: true})
...
export const singIn: RequestHandler = async (req, res) => {
  ...
  if (!user) return sendErrorRes(res, "Email/Passoword mismatch", 403)

  const isMatched = await user.comparePassword(password)
  if (!isMatched) return sendErrorRes(res, "Email/Passoword mismatch", 403)
...
export const grantAcessToken: RequestHandler = async (req, res) => {
  ...
  const filtredTokens = user.tokens.filter((t) => t !== refreshToken)
  user.tokens = filtredTokens
